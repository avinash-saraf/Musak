...
def show_text_summary_page():
    ...
    summary_display_format = st.radio("Summary Display Format:",["Paragraph","Bullet Points"])
    st.text(" ")
    ...
    if make_summary:
        with st.spinner(text="Generating Summary...."):
            ...
            if msg[0] == "Error":
                ...
                summary_for_tts, summary = generate_ext_summary(msg[1]) if summary_type == "Extract" else generate_abs_summary(msg[1])
...
def generate_abs_summary(TEXT):
    ...
    try:
        ...
        text_for_tts = final_summary = ' '.join(summary['summary_text'] for summary in summary_dict)
        if summary_display_format != "Paragraph":
            final_summary = '\n'.join(f"* {line}." if line != "" else "" for line in final_summary.split("."))
